...
function loadStore(){
  try{
    ...
    if(!raw){
      return { kids: {}, transactions: {}, lastKidId: null };
    }
    ...
    console.warn("스토어 로드 실패, 초기화합니다.", e);
    return { kids: {}, transactions: {}, lastKidId: null };
  }
}
function saveStore(){
...
function renderKidTabs(){
  ...
  if(ids.length === 0){
    ...
    li.innerHTML = `<div class="kid-card">
      ...
      <div class="kid-meta">
        <div class="kid-name">아직 아이가 없어요</div>
        <div class="kid-sub">[+ 아이 추가]를 눌러 시작해요</div>
      </div>
    </div>`;
  ...
  ids.forEach(id => {
    ...
    const li = document.createElement('li');
    li.innerHTML = `<div class="kid-card ${id===currentKidId ? 'active':''}" data-id="${id}" style="--kid:${CSS.escape(k.color||'#ffe3ef')}">
    ...
    // 클릭 전환
    li.querySelector('.kid-card').addEventListener('click', () => {
      selectKid(id);
    });
    // 길게 눌러 컨텍스트 메뉴
    li.querySelector('.kid-card').addEventListener('pointerdown', (e) => {
      pressedKidId = id;
      longPressTimer = setTimeout(() => {
        openKidMenu(e.clientX, e.clientY, id);
      }, 650);
    });
    li.querySelector('.kid-card').addEventListener('pointerup', () => {
      clearTimeout(longPressTimer);
    });
    li.querySelector('.kid-card').addEventListener('pointerleave', () => {
      clearTimeout(longPressTimer);
    });
    kidTabs.appendChild(li);
  });
...
function kidBalance(id){
  const arr = store.transactions[id] || [];
...
function renderOverview(){
  ...
  const kid = store.kids[currentKidId];
  const arr = store.transactions[currentKidId] || [];
  const now = new Date();
  ...
  btnPayAllowance.disabled = !kid.weekly;
}
...
function renderTxList(){
  ...
  const ymFilter = filterMonth.value;
  const typeFilter = filterType.value;
  const catFilter = filterCat.value;

  let count = 0;
  for(const t of arr){
    if(ymFilter && !(t.date||"").startsWith(ymFilter)) continue;
  ...
  // 액션 연결
  txList.querySelectorAll('[data-act="del"]').forEach(btn => {
    btn.addEventListener('click', () => {
      const id = btn.dataset.id;
      delTx(id);
    });
  });
  txList.querySelectorAll('[data-act="edit"]').forEach(btn => {
    btn.addEventListener('click', () => {
      const id = btn.dataset.id;
      editTx(id);
    });
...
function openKidModal(editId=null){
  ...
  if(editId){
    ...
    kidName.value = "";
    kidEmoji.value = "";
    kidColor.value = "#ffd9e6";
    kidWeekly.value = "";
    delete kidForm.dataset.editId;
  }
  if (typeof kidModal.showModal === 'function') {
    kidModal.showModal();
  } else {
    alert('이 브라우저는 dialog를 지원하지 않아요. 다른 브라우저를 사용해 주세요.');
  }
}
kidForm.addEventListener('submit', (e) => {
  ...
  const data = {
    ...
    weekly: Math.max(0, Number(kidWeekly.value || 0)|0),
  };
  const editId = kidForm.dataset.editId;
  if(editId){
    store.kids[editId] = {...store.kids[editId], ...data};
    saveStore();
    kidModal.close();
    renderKidTabs();
    if(editId === currentKidId){ renderOverview(); }
  }else{
    const id = uid("kid_");
    store.kids[id] = { id, ...data };
...
function openKidMenu(x, y, id){
  ...
  kidDeleteBtn.onclick = () => {
    if(confirm("이 아이와 모든 거래 기록을 삭제할까요? 이 작업은 되돌릴 수 없어요.")){
      delete store.kids[id];
      delete store.transactions[id];
      if(currentKidId === id){
        currentKidId = Object.keys(store.kids)[0] || null;
        store.lastKidId = currentKidId;
      }
      saveStore();
      kidMenu.close();
...
txForm.addEventListener('submit', (e) => {
  e.preventDefault();
  if(!currentKidId){ alert("먼저 아이를 추가해 주세요!"); return; }
  ...
  store.transactions[currentKidId].push(tx);
  saveStore();
  txForm.reset();
  txDate.valueAsDate = new Date();
  renderOverview();
...
// 용돈 지급
btnPayAllowance.addEventListener('click', () => {
  if(!currentKidId) return;
  const kid = store.kids[currentKidId];
  const w = kid.weekly || 0;
...
btnExport.addEventListener('click', () => {
  ...
  a.download = `kiddy-budget-${ts}.json`;
  a.click();
...
importFile.addEventListener('change', (e) => {
  ...
  reader.onload = () => {
    try{
      ...
      store = obj;
      currentKidId = store.lastKidId || Object.keys(store.kids)[0] || null;
      saveStore();
      renderKidTabs(); renderOverview(); renderFilters(); renderTxList();
      alert("가져오기가 완료되었습니다!");
    }catch(err){
      alert("가져오기에 실패했어요. JSON 형식을 확인해 주세요.");
      console.error(err);
    }
  };
...
btnAddDemo.addEventListener('click', () => {
  if(Object.keys(store.kids).length > 0){
    if(!confirm("이미 데이터가 있어요. 그래도 데모 데이터를 합칠까요?")) return;
  }
  const kid1 = { id: uid("kid_"), name: "유겸", emoji:"🐰", color:"#ffd9e6", weekly: 2000 };
  const kid2 = { id: uid("kid_"), name: "지안", emoji:"🦄", color:"#d9fff2", weekly: 1500 };
  store.kids[kid1.id] = kid1;
  store.kids[kid2.id] = kid2;
  store.transactions[kid1.id] = [
