...
function renderKidTabs(){
  ...
  ids.forEach(id=>{
    ...
    const li = document.createElement('li');
    li.innerHTML = `
      <div class="kid-card ${id===currentKidId?'active':''}" data-id="${id}">
    ...
    // 길게 눌러 컨텍스트 메뉴
    let t=null;
    card.addEventListener('pointerdown',(e)=>{ t=setTimeout(()=>openKidMenu(e.clientX,e.clientY,id),650); });
...
function selectKid(id){
  currentKidId = id; store.lastKidId = id; saveStore();
...
function openKidModal(editId=null){
  const title = editId ? "아이 수정" : "아이 추가";
  kidModalTitle.textContent = title;
  if(editId){
    const k = store.kids[editId];
    kidName.value = k.name||""; kidEmoji.value = k.emoji||""; kidColor.value = k.color||"#ffd9e6"; kidWeekly.value = k.weekly||0;
...
kidForm.addEventListener('submit', (e)=>{
  ...
  if(editId){
    ...
    saveStore(); kidModal.close(); renderKidTabs(); if(editId===currentKidId) renderOverview();
  }else{
    const id = uid("kid_");
    store.kids[id] = { id, ...data };
    store.transactions[id] = [];
    currentKidId = id; store.lastKidId = id; saveStore();
    kidModal.close(); renderKidTabs(); selectKid(id);
  }
});
...
txForm.addEventListener('submit', (e)=>{
  ...
  saveStore(); txForm.reset(); txDate.valueAsDate = new Date(); renderOverview(); renderTxList();
});
...
function delTx(id){
  ...
  if(idx>=0){ arr.splice(idx,1); saveStore(); renderOverview(); renderTxList(); }
}
function editTx(id){
...
importFile.addEventListener('change', (e)=>{
  ...
  reader.onload = ()=>{
    try{
      ...
      saveStore(); renderKidTabs(); renderOverview(); renderFilters(); renderTxList();
      alert("가져오기가 완료되었습니다!");
    }catch(err){
      alert("가져오기에 실패했어요. JSON 형식을 확인해 주세요."); console.error(err);
    }
  };
...
function todayOffset(d=0){ const x=new Date(); x.setDate(x.getDate()+d); return x.toISOString().slice(0,10); }
